...
class Table
{
    ...
    /**
     * Inserts many rows with as few statements as possible.
     *
     * Rows that set the same columns share multi-row INSERT statements of up
     * to $batch_size rows, fewer when the adapter limits the bound parameters
     * per statement. Values get the same conversions as insert() (dates, JSON)
     * and missing created_at/updated_at columns are filled in, but no model is
     * built: validations and callbacks do not run.
     *
     * @param array $rows List of [attribute => value] hashes
     * @param int|null $batch_size Rows per statement, the adapter's $INSERT_BATCH_SIZE if null
     * @return int Number of rows sent to the database
     */
    public function insert_all(array $rows, $batch_size = null)
    {
        $conn = $this->conn;
        $batch_size = (int)($batch_size ?? $conn::$INSERT_BATCH_SIZE);

        if ($batch_size < 1) {
            throw new ActiveRecordException('insert_all() needs a batch_size of at least 1');
        }

        $timestamps = [];
        foreach (['created_at', 'updated_at'] as $name) {
            if (($column = $this->get_column_by_inflected_name($name))) {
                $timestamps[$name] = $column->name;
            }
        }

        // an object, so that each adapter writes it in its own format
        $now = new \DateTime();
        $pk = $this->sequence && $this->pk ? $this->column_name_for($this->pk[0]) : null;
        $groups = [];

        foreach ($rows as $row) {
            if (!is_array($row)) {
                throw new ActiveRecordException('insert_all() takes a list of [attribute => value] hashes');
            }

            foreach ($timestamps as $name => $column_name) {
                if (!array_key_exists($name, $row) && !array_key_exists($column_name, $row)) {
                    $row[$name] = $now;
                }
            }

            $data = $this->process_data($row) ?: [];

            // a missing or null key is generated by the adapter, as in Model::insert()
            $generate = $pk && !isset($data[$pk]);
            if ($generate) {
                unset($data[$pk]);
            }

            // the same columns in any order go in the same statement
            ksort($data);
            $key = ($generate ? '+' : '-') . implode("\0", array_keys($data));

            $groups[$key]['columns'] ??= array_keys($data);
            $groups[$key]['sequence'] ??= $generate ? [$pk, $this->sequence] : null;
            $groups[$key]['rows'][] = array_values($data);
        }

        $table = $this->get_fully_qualified_table_name();

        foreach ($groups as $group) {
            $per_statement = intdiv($conn->max_bind_parameters(), max(1, count($group['columns'])));
            $per_statement = max(1, min($batch_size, $per_statement));

            foreach (array_chunk($group['rows'], $per_statement) as $chunk) {
                $conn->insert_rows($table, $group['columns'], $chunk, $group['sequence']);
            }

            $this->last_sql = $conn->last_query;
        }
    ...
    private function set_sequence_name()
    {
        ...
        // a table without a primary key, common in ClickHouse, has nothing to generate
        if (!($this->sequence = $this->class->getStaticPropertyValue('sequence')) && $this->pk) {
            $pk = $this->pk[0];
            $column = $this->columns[$pk] ?? $this->get_column_by_inflected_name($pk);
            $this->sequence = $this->conn->get_sequence_name($this->table, $pk, $column);
        }
    }
